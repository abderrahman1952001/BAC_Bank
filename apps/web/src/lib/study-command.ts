import type {
  StudyCommandCreateSessionRequest,
...
function topicSubjectMatches(
  ...
  return (
    topic.subject.code === subject.code || topic.subject.family?.code === subject.code
  );
}
...
function subjectReferenceMatches(
  ...
  return subjectRef.code === subject.code || subjectRef.family?.code === subject.code;
}
...
function topicFromFilters(
  ...
  const topics = context?.filters?.topics.filter((topic) =>
...
export function inferStudyCommandMode(
  ...
  if (
    includesAny(normalized, ["مختبر", "lab", "تجربة", "visualiser", "رسم"])
  ) {
    return "LAB_EXPLORATION";
  }

  if (
    includesAny(normalized, ["مواضيع", "باك", "paper", "archive", "أرشيف"])
  ) {
    return "BAC_TRAINING";
  }
...
function hasCatalogCoverage(input: {
  ...
  const streams = userStreamCode
    ? input.context.catalog.streams.filter((stream) => stream.code === userStreamCode)
...
function buildProposalAction(input: {
  ...
  const topicCodes =
    input.topic?.canUseCode && input.topic.code ? [input.topic.code] : undefined;
...
export function buildStudyCommandMixedDrillFallbackRequest(
  request: StudyCommandCreateSessionRequest,
): StudyCommandCreateSessionRequest | null {
  if (!request.topicCodes?.length) {
    return null;
  }
...
export function buildStudyCommandProposal(
  ...
  if (mode === "CONTINUE_SESSION" && activeSession) {
    const title = activeSession.title ?? formatStudySessionKind(activeSession.kind);
  ...
  return {
    ...
    primaryHref,
    primaryLabel:
      primaryAction.kind === "OPEN_ROUTE" && canCreateStudySessionFromMode(mode)
        ? "ضبط الجلسة"
    ...
    primaryAction,
    steps: buildProposalSteps({ mode, subjectName, topic: topicName }),
    fineTuneOptions: buildFineTuneOptions(mode),
  };
}
...
export function buildStudyCommandStarters(
  ...
  const labTool =
    ...
    context.labTools.find((tool) => tool.completedMissionCount < tool.missionCount) ??
    null;
  ...
  if (starters.length === 0) {
    starters.push(
      {
        id: "fallback-school-test",
        title: "حضّر فرض قريب",
        prompt: "عندي فرض قريب وأريد جلسة مراجعة قصيرة",
        reason: "ابدأ من وضعك الدراسي الآن",
        tone: "primary",
        mode: "SCHOOL_TEST_PREP",
        href: STUDENT_TRAINING_ROUTE,
      },
      {
        id: "fallback-bac-training",
        title: "تدريب BAC مركز",
        prompt: "أريد تمارين BAC في مادة أختارها الآن",
        reason: "أسرع مدخل للتدريب الجاد",
        tone: "neutral",
        mode: "BAC_TRAINING",
        href: STUDENT_TRAINING_ROUTE,
      },
