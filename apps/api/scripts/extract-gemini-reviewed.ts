...
const SYSTEM_INSTRUCTION = [
  ...
  'For simple numeric tables, correction tables, sign tables, variation tables, truth tables, and probability trees, add nativeSuggestion by default whenever every cell/branch/label/value is visibly present and can be transcribed faithfully.',
  'Treat tables, probability trees, sign tables, and variation tables as native-first, not crop-first. Do not leave obvious native-renderable structures as image/crop-only work for human crop review.',
  'Native table suggestions must use data.rows as an array of row arrays. Put KaTeX-compatible math inside cells using $...$ where useful, especially for sign and variation tables when it best preserves the source layout.',
...

  {
    ...
    draft.exam.metadata = {
      ...
      nativeRenderingPolicy:
        'Gemini should suggest native tables and probability trees whenever they are faithful and fully visible; graphs remain image assets pending future renderer support.',
...
function buildUserPrompt(target: SourceTarget) {
  return [
    ...
    'For sign tables, variation tables, simple tables, correction tables, truth tables, and probability trees, include nativeSuggestion by default when faithful and fully visible.',
    'Do not make obvious native-renderable tables or trees human crop work unless native transcription is blocked by real ambiguity or unreadability.',
    '',
