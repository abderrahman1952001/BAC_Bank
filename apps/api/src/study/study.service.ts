...
export class StudyService {
  constructor(
    ...
    private readonly examActivityService: StudyExamActivityService,
    private readonly studyQuestionAiExplanationService: StudyQuestionAiExplanationService,
  ) {}
  ...
  async listRecentStudySessions(
    ...
    return this.studySessionService.listRecentStudySessions(userId, limit);
  }
  ...
  async createStudySession(
    ...
    return this.studySessionService.createStudySession(userId, payload);
  }
  ...
  async getStudySessionById(
    ...
    return this.studySessionService.getStudySessionById(userId, id);
  }

  async getStudyQuestionAiExplanation(
    userId: string,
    sessionId: string,
    questionId: string,
    ...
    if (user.subscriptionStatus !== SubscriptionStatus.ACTIVE) {
      throw new ForbiddenException(
        'AI explanation is available only for premium students.',
      );
    }

    const session = await this.studySessionService.getStudySessionById(
      userId,
      sessionId,
    );
    ...
    if (
      ...
      effectiveStatus !== StudySessionStatus.EXPIRED
    ) {
      throw new ForbiddenException(
        'AI explanation is available only after the simulation ends.',
      );
    }

    const exercise = session.exercises.find((entry) =>
      entry.hierarchy.questions.some((question) => question.id === questionId),
    );
    const question = exercise?.hierarchy.questions.find(
    ...
    return this.studyQuestionAiExplanationService.generate({
      questionId,
      subjectName: exercise.exam.subject.name,
      supportStyle: session.pedagogy.supportStyle,
  ...
  async updateStudySessionProgress(
    ...
    return this.studySessionService.updateStudySessionProgress(
      userId,
      id,
      payload,
    );
